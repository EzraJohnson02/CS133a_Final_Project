...
class Trajectory:
    # Initialization.
    def __init__(self, node, p_ball, v_ball, a_ball, p_ball2, v_ball2, a_ball2):
    ...
    def set_goal(self, p_ball, v_ball, a_ball, p_ball2, v_ball2, a_ball2):
        min_time = 0.2
        tp = TrajectoryPlanner(p_ball, v_ball, a_ball, min_time)
    ...
    def evaluate(self, t, dt):
        ...
        Jp2 = np.transpose(J2) @ np.linalg.pinv(J2 @ np.transpose(J2) + gamma ** 2 * np.eye(M))
        qdot2 = Jp2 @ xrdot2
        qdot = Jp @ xrdot + (np.eye(N) - Jp @ J) @ qdot2
