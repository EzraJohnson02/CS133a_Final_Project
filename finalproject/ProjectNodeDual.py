...
class ProjectNode(Node):
    # Initialization.
    def __init__(self, name, p, v, rate, Trajectory1, Trajectory2):
        ...
        self.p2 = np.array([p[0], p[1], p[2]]).reshape((3,1))
        self.v2 = np.array([-v[0], -v[1], -v[2]]).reshape((3,1))
        ...
        # Set up a trajectory.
        self.trajectory1 = Trajectory1(self, self.p, self.v, self.a)
        self.trajectory2 = Trajectory2(self, self.p2, self.v2, self.a2)
        self.turn = 1
        # self.trajectory = Trajectory(self, self.p, self.v, self.a, self.p2, self.v2, self.a2)
        self.tr = Trajectory1
        self.jointnames = self.trajectory1.jointnames()
        ...
        self.t     = -self.dt
        self.t_trajectory1 = -self.dt
        self.t_trajectory2 = -self.dt
    ...
    def update(self):
        ...
        desired = self.trajectory1.evaluate(self.t_trajectory1, self.dt)
        if desired is not None:
            ...
            if not (isinstance(q, list) and isinstance(qdot, list)):
                ...
                return
            if not (len(q) == len(self.trajectory1.jointnames())):
            ...
            cmdmsg.name         = self.trajectory1.jointnames()   # List of joint names
            cmdmsg.position     = q                 # List of joint positions
            cmdmsg.velocity     = qdot              # List of joint velocities
            self.pub.publish(cmdmsg)

        desired = self.trajectory2.evaluate(self.t_trajectory2, self.dt)
        if desired is not None:
            (q, qdot) = desired
            # Check the results.
            if not (isinstance(q, list) and isinstance(qdot, list)):
                self.get_logger().warn("(q) and (qdot) must be python lists!")
                return
            if not (len(q) == len(self.trajectory2.jointnames())):
                self.get_logger().warn("(q) must be same length as jointnames!")
                return
            if not (len(q) == len(self.trajectory2.jointnames())):
                self.get_logger().warn("(qdot) must be same length as (q)!")
            ...
            cmdmsg = JointState()
            cmdmsg.header.stamp = now.to_msg()      # Current time for ROS
            cmdmsg.name         = self.trajectory2.jointnames()   # List of joint names
            cmdmsg.position     = q                 # List of joint positions
            cmdmsg.velocity     = qdot              # List of joint velocities
            self.pub.publish(cmdmsg)
        ...
        if self.completed2:
            self.t_trajectory2 = 0.
            self.completed2 = False
            self.trajectory2.set_goal(self.p2, self.v2, self.a2)

        if self.completed1:
            self.t_trajectory1 = 0.
            self.completed1 = False
            self.trajectory1.set_goal(self.p, self.v, self.a)

        # Check for a bounce - not the change in x velocity is non-physical.
        p1_tip = self.trajectory1.position()
        p2_tip = self.trajectory2.position()
        if np.linalg.norm(self.p - p1_tip) < 0.2:
            self.v *= -1.
            self.completed1 = True
        ...
        if np.linalg.norm(self.p2 - p2_tip) < 0.2:
            self.v2 *= -1.
            self.completed2 = True
        elif self.p2[2, 0] < self.radius + (-1.4):
            self.p2[2,0] = self.radius - 1.4
            self.p2 -= 0.1 * self.v2
            self.v2 *= -1.
            self.completed2 = True
